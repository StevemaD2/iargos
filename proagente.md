...
## Visão geral
- **Stack**: Vite + React (TS). Rotas via `HashRouter`. Integrações: Supabase (DB + Storage), Mapbox (Zone Planner), Gemini (insights via `API_KEY`).
- **Principais componentes**:
  - `App.tsx`: roteamento com `Dashboard` (views `COMMAND`, `STRUCTURE`, `CRONOGRAMA`, `SETTINGS`, `FINANCE`, `TEAM`, `VOTERS`, `LEADER_TEAM`), `SubmissionForm`, `LeaderFeed` e `CandidateShare`. Rotas públicas: `/`, `/diretor`, `/santinho/:slug`.
  - `Login.tsx`: autenticação separada para diretor (`/#/diretor`) e subordinados (`/#/`), usando `initialMode`/`lockMode`. Onboarding por convite com CPF + PIX e login por CPF.
  - `Dashboard.tsx`: módulos (Comando Central com insights, Estrutura & Avisos/Zone Planner, Controle & Financeiro, Cronograma, Configurações, Expansão, Eleitores, Minha Equipe).
  - `ZonePlanner.tsx`: gestão de zonas/subzonas com Mapbox, ações por subzona e atribuição de líderes.
  - `CandidateShare.tsx`: “santinho digital” público via `/#/santinho/:slug`.
- **Serviços/config**: `services/operationSettingsService.ts`, `memberActivityService.ts` (folha de ponto + diárias), `locationService.ts`, `submissionsService.ts` (registros + anexos), `votersService.ts` (eleitores), `geminiService.ts` (insights).

## Estrutura da base (Supabase)
- `operacoes`: operação ativa/inativa, `slug`, estado e vínculo com `diretor_id`; também guarda dados do candidato (`candidate_*`), links, destaques e cores do tema (`theme_primary_color`, `theme_secondary_color`, `campaign_type`).
...
- `zonas` ↔ `subzonas`: recortes geográficos por operação (nome, descrição, `cor`, `poligono_geojson`, `ordem`); cada subzona pode ter ações e líderes.
- `subzona_acoes`: tarefas registradas em zona ou subzona (data, título, descrição, `status`, `responsavel_id`, observações).
...
- Comando Central carrega submissões da operação e gera gráficos de sentimento/bairros; se `API_KEY` do Gemini estiver disponível, cria resumo/temas e briefing do candidato (diretor).
...
- Estrutura & Avisos inclui Zone Planner com zonas/subzonas, ações e atribuição de líderes (Mapbox).
...
- Eleitores: cadastro e listagem por operação; líderes enxergam subset filtrado pela cadeia.
- Configurações permite editar operação, cronograma e dados do candidato + tema do santinho digital (gera link `/#/santinho/:slug` com prévia).
...
- Monitoramento (`/feed`) ainda usa dados mockados.
...
2. **Backlog crítico**:
   ...
   - Integrar upload/arquivos com impacto na folha de ponto (reset de contador e visibilidade no histórico).
   - Tornar o monitoramento (`/feed`) dinâmico (hoje é mock).
3. **Tokens/keys**:
   - `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` obrigatórios.
   - `VITE_MAPBOX_TOKEN` para Zone Planner.
   - `API_KEY` (Gemini) para insights no Comando Central.
4. **Multi-tenant**: sempre filtrar consultas por `operacao_id` e, futuramente, habilitar RLS no Supabase.
5. **Documentos úteis**:
...
3. Confirmar `.env.local` (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_MAPBOX_TOKEN`, `API_KEY`) se for testar convites/logins e Zone Planner.
...
- Backend para uploads (com storage por operação) + reflexo na folha de ponto.
